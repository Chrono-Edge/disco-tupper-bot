...
class TupperCommandsCog(commands.Cog):
    ...
    async def _get_user_to_edit_tupper(
        ...
        if await Permissions.get_user_is_admin(self.admin_roles, ctx) and member:
            return member, await User.get_or_create(discord_id=member.id)
        return ctx.author, await User.get_or_create(discord_id=ctx.author.id)
    ...
    async def create_tupper(
        ...
        if await user.tuppers.filter(call_pattern=call_pattern):
    ...
    @commands.hybrid_command(name="set_tupper_avatar")
    @commands.has_any_role(*config.player_roles)
    async def set_tupper_avatar(
        self,
        ctx,
        name: str,
        avatar: discord.Attachment,
    ...
    async def send_balance(
        ...
        to_member: typing.Optional[discord.Member],
        to_tupper_name: str,
        amount: int,
    ):
        ...
        _, to_user = await self._get_user_to_edit_tupper(ctx, to_member)
        ...
        balance = from_tupper.balance - amount
        await from_tupper.update(balance=balance)
